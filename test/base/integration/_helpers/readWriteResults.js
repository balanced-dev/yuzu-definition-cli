...
const deleteFile = (testName, actualSuffix, ext) => {
    var filename = `${resultsDir}${path.sep}${path.sep}${testName}_${actualSuffix}.${ext}`;
    if(fs.existsSync(filename)) fs.unlinkSync(filename);
};

const write = (testName, content, suffix, ext) => {
    fs.writeFileSync(`${resultsDir}${path.sep}${path.sep}${testName}_${suffix}.${ext}`, content);
}

const read = (testName, suffix, ext) => {
    return fs.readFileSync(`${resultsDir}${path.sep}${path.sep}${testName}_${suffix}.${ext}`, 'utf8');
}
