...
const formatScss = function(start, lines) {
    let returnString = start,
        lineIndent = '',
        indentChars = ' '.repeat(base.settings.markupSettings.indentSize);
...
describe('Scss service integration tests', function() {
	...
	it('Creates simple, single property stylesheeet', function() {
        ...
        var html = [
            `{{#if text}}`,
                `<${base.settings.markupSettings.defaultMarkupTag} class="${base.cardSettings.className}__text">`,
                    `{{text}}`,
                `</${base.settings.markupSettings.defaultMarkupTag}>`,
            `{{/if}}`
        ];
	...
	it('Creates nested stylesheeet', function() {
        ...
        var html = [
            `<${base.settings.markupSettings.defaultMarkupTag} class="${base.cardSettings.className}__obj">`,
                `{{#if obj.text}}`,
                    `<${base.settings.markupSettings.defaultMarkupTag} class="${base.cardSettings.className}__obj__text">`,
                        `{{text}}`,
                    `</${base.settings.markupSettings.defaultMarkupTag}>`,
                `{{/if}}`,
            `</${base.settings.markupSettings.defaultMarkupTag}>`
	...
	it("Doesn't pollute class name", function() {
        ...
        var html = [
            `<${base.settings.markupSettings.defaultMarkupTag} class="${base.cardSettings.className}__obj {{#each _modifiers}} {{this}}{{/each}}">`
        ];
	...
	it("Doesn't include all classes on element (just first)", function() {
        ...
        var html = [
            `<${base.settings.markupSettings.defaultMarkupTag} class="${base.cardSettings.className}__obj-test box additional-class">`
        ];
